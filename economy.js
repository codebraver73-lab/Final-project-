...
let storage1=JSON.parse(localStorage.getItem("storage1")) || [];
...
function updatingUI(comment){
    ...
    storage1 = storage1.filter(t => t !== comment);
    updatingls();
...
storage1.forEach((comment)=>{updatingUI(comment)})
...
let newsobtaning={
    news: function(){
        let page1Results;
        fetch("https://newsdata.io/api/1/latest?apikey=pub_79bbad64eb794cc89a8470b63a074012&language=en&category=business&image=1&removeduplicate=1")
        .then((response)=>{
            if(!response.ok){
        ...
        .then((data)=>{
            ...
            return fetch("https://newsdata.io/api/1/latest?apikey=pub_79bbad64eb794cc89a8470b63a074012&language=en&category=business&image=1&removeduplicate=1&page="+nextPage)})       
...
let videos = {
  fetching_videos: function() {
    fetch("https://www.googleapis.com/youtube/v3/search?part=snippet&q=business%20news&type=video&maxResults=8&relevanceLanguage=en&videoEmbeddable=true&videoSyndicated=true&eventType=completed&videoDefinition=high&key=AIzaSyA_UaEhpijMs-7Gmawqk5EGLCDRsN0GMZ0")
      .then((response) => {
        if (!response.ok) {
